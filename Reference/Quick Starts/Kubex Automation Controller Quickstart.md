#kubex #k8s #tutorial #howto #quickstart

## Delete cert-manager remnants
...
densifyCredentials:
  username: 'dchase@densify.com' # Username to access Densify, mandatory if createSecrets is true
...
valkey:
  credentials:
    password: 'xH7#KJRlJ^' # valkey password, mandatory if createSecrets is true,
...
cluster:
  name: demo01   # [REPLACE] Kubernetes cluster name

certmanager:
  enabled: true        # Set to true if using cert-manager
  ...
  - name: base-optimization-scope # Unique scope name
    ...
    podLabels:            # Optional - defines which pods to include/exclude based on labels
      - key: app        # Replace with actual label key (e.g., "app", "env", "tier")
        operator: NotIn              # Options: In | NotIn  
        values:
          - impossiblevalue     # Replace with actual label values
...
policy:
  ...
  policies: 
    base-optimization:
      allowedPodOwners: "Deployment,CronJob,Rollout,Job,ReplicaSet,AnalysisRun"
      enablement:
        cpu:
          request:
            downsize: true
...
#    podScanInterval: "3m"                     # How often to scan cluster for pods to check 
#    podScanTimeout: "1m"                      # Max time allowed for each pod scanning cycle  
