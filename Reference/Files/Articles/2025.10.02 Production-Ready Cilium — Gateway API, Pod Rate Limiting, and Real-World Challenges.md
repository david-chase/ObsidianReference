...
https://medium.com/aws-in-plain-english/production-ready-cilium-gateway-api-pod-rate-limiting-and-real-world-challenges-b4add190d66d
...
In Parts 1–3, we explored Cilium’s origins, technical architecture, and observability capabilities. We’ve seen the theory, the architecture diagrams, and the lab examples. Now it’s time to address the question every platform engineer asks: **“How do I actually run this in production?”**

Production Kubernetes deployments face challenges that don’t appear in demos:
...
This final article focuses on production-grade features that separate successful deployments from troubled ones. We’ll cover:

**Gateway API**: The next-generation ingress with TLS termination, traffic splitting, and multi-tenant separation — all without sidecars.
...
**Real-World Challenges**: Battle-tested strategies from companies running Cilium at scale — what works, what doesn’t, and how to avoid common pitfalls.

**What makes this different from Parts 1–3**: This isn’t about how Cilium works internally — it’s about how to operate it successfully. You’ll learn from actual production deployments at companies like Form3, IKEA, Palantir, and others who’ve navigated the journey from proof-of-concept to mission-critical infrastructure.

**🔗 GitHub Repository**: All manifests, Grafana dashboards, and troubleshooting guides:  
[**cilium-deep-dive-labs**](https://github.com/Salwan-Mohamed/cilium-deep-dive-labs) — See `labs/05-production-features/` for complete examples.

Let’s dive into production-ready Cilium.

Press enter or click to view image in full size

![](https://miro.medium.com/v2/resize:fit:831/0*KOUBNheeOnVk_949)

## Chapter 1: Gateway API — Modern Ingress for Production

## Why Gateway API Matters

Traditional Kubernetes Ingress has served us well, but it has limitations:

- **Single-layer configuration**: Can’t separate platform and application concerns
...
Let’s walk through a production-grade HTTPS ingress setup.
...
- `https://bookinfo.cilium.rocks/details` → details service
- `https://bookinfo.cilium.rocks/reviews` → reviews service
...
**Certificate verified, TLS terminated, traffic routed** — all with zero application changes.
...
**🔗 Repository**: Complete Gateway API examples in `labs/05-production-features/gateway-api/`

## Chapter 2: Pod Rate Limiting — Traffic Management at Scale
...
**Cilium’s solution**: eBPF-based **bandwidth management** that enforces per-pod rate limits at the kernel level with negligible overhead.
...
**The limit is enforced in eBPF** — kernel-level, per-identity, with minimal CPU overhead.
...
**Multi-Tenant SaaS**: Prevent one customer’s workload from starving others **Data Processing**: Limit batch job bandwidth to protect real-time services **Cost Control**: Prevent runaway data transfer costs in cloud environments
...
**🔗 Repository**: Rate limiting examples and testing tools in `labs/05-production-features/rate-limiting/`

## Chapter 3: Grafana Monitoring — Production Observability
...
**The stack**: Cilium → Prometheus → Grafana
...
**BPF Map Pressure**: Critical metric — full maps cause packet drops
...
**🔗 Repository**: Complete Grafana dashboards and alert rules in `labs/05-production-features/grafana-monitoring/`
...
**Key Lesson**: **Start permissive, observe, then restrict**. Don’t go zero-trust day one.
...
**Key Lesson**: **Cluster Mesh isn’t magic** — you still need proper routing between clouds (VPN, Direct Connect). Cilium handles the rest.
...
**2. Hubble Relay Overload**: Central relay couldn’t keep up with flow volume from all nodes.
...
**Key Lesson**: **Cilium scales, but you must tune it**. Default settings are for 50–100 node clusters.
...
Hubble Flows → Timescape (ClickHouse) → Daily S3 Export → Glacier  
              ↓  
         Grafana (Ops)  
              ↓  
...
**Key Lesson**: **Compliance is built-in with Hubble** — you just need to export and retain the data.
...
Don’t deploy Cilium cluster-wide immediately. Start small:
...
**3. Production — one namespace**

- Low-risk application
- Monitor closely for 1 week

**4. Production — gradual expansion**
...
- Legacy CNI for “boring” workloads
